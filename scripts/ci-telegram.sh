#!/usr/bin/env bash
# Q4 Telegram collage: dry-run (PR) / live (main + workflow_dispatch).
...
# Pin: npx @qa-guru/allure-notifications@6.1.1 (kit profile dogfood collage).
set -euo pipefail
...
if [[ ! -f allure-report/summary.json ]]; then
  echo "ERROR: allure-report/summary.json missing — generate this-run report first (npm test && npm run allure:generate)" >&2
...
cfg["base"]["project"] = f"allure-report-kit · {ref} · {sha}"
...
project_id = (os.environ.get("ALLURE_PROJECT_ID") or "5321").strip()
testops = f"{endpoint}/project/{project_id}" if project_id else f"{endpoint}/project/5321"
links.setdefault("report", testops)
links.setdefault("testops", testops)
links.setdefault(
    "dashboard",
    os.environ.get("SONAR_DASHBOARD_URL")
    or "https://github.com/qa-guru/allure-report-kit/actions",
)
if build:
    links["build"] = build
else:
    links.setdefault(
        "build",
        "https://github.com/qa-guru/allure-report-kit/actions",
    )
