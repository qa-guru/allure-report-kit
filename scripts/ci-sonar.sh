#!/usr/bin/env bash
# Sonar upload + quality gate poll for allure-report-kit (Q2 + Q5 harden).
...
export SONAR_PROJECT_KEY="${SONAR_PROJECT_KEY:-allure-report-kit}"
...
if [[ ! -f coverage/lcov.info ]]; then
  required_or_soft "coverage/lcov.info missing — run npm run coverage / download artifact first"
fi
