#!/usr/bin/env node
/**
 * Gate: every *-result.json must carry explicit Allure suite labels.
 * Required: epic, feature, story, layer, severity.
 * Also required when layer is e2e: label component.
 */
...
const COMPONENT_LAYERS = new Set(["e2e"]);
...
function labelMap(raw) {
  ...
  for (const row of labels) {
    ...
    const { name, value } = /** @type {{ name?: string; value?: string }} */ (row);
...
export async function checkAllureLabels(resultsDir) {
  ...
  for (const file of files) {
    ...
    if (layer && COMPONENT_LAYERS.has(layer) && !labels.component?.trim()) {
      failures.push(`${name}: missing label "component" for layer=${layer} (${file})`);
    }
  }
